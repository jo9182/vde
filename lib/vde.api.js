let VDE = {
    ...
    async getDocumentFile(path, format = null) {
        return await this.getFile(path, 'document', format);
    },
    async getPublicFile(path, format = null) {
        return await this.getFile(path, 'public', format);
    },
    async getInternalFile(path, format = null) {
        return await this.getFile(path, 'internal', format);
    },
    async getStorageFile(path, format = null) {
        return await this.getFile(path, 'storage', format);
    },
    async getFile(path, location = 'storage', format = null) {
        let resource = null;
        ...
        if (location === 'document') resource = await this.getRemoteFile(`/docs/${path}`);
